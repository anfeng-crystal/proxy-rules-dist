...
- 远程规则 ID、文件名、目录名和 provider 名统一使用英文：`Domestic`、`NetworkTest`、`SpeedTest`、`Gemini`、`AI`、`Apple`、`Microsoft`、`Google`、`GitHub`、`Dev`、`Payment`、`Game`、`Telegram`、`YouTube`、`Netflix`、`DisneyPlus`、`Spotify`、`TikTok`、`Twitter`、`Facebook`、`Instagram`、`GlobalSites`、`Ads`。
- 配置里的策略组展示名继续使用中文或品牌名：`国内应用`、`网络检测`、`SpeedTest`、`Gemini`、`AI`、`Apple`、`Microsoft`、`Google`、`GitHub`、`Dev`、`Payment`、`Game`、`Telegram`、`YouTube`、`Netflix`、`DisneyPlus`、`Spotify`、`TikTok`、`Twitter`、`Facebook`、`Instagram`、`境外网站`、`Ads`、`漏网之鱼`。
...
- `Gemini` 单独发布 Google Gemini、AI Studio 和 NotebookLM 规则；同一批规则仍保留在完整的 `AI` 聚合中。`Payment` 只聚合海外支付金融；`Ads` 聚合广告、隐私和劫持拦截类。
