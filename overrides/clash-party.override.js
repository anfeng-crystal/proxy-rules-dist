// RuleForge Clash Party JavaScript override.
...
function main(config) {
  ...
  function preserveRequiredOriginalGroups(groups) {
    ...
    // A generated RuleForge name always wins; the airport group is aliased on collision.
    for (const name of required) {
      if (!generatedNames.has(name)) {
        stableNameByOriginal.set(name, name);
        continue;
      }
      ...
      stableNameByOriginal.set(name, alias);
      reservedNames.add(alias);
    }

    function stableName(name) {
      return typeof name === "string" && stableNameByOriginal.has(name)
        ? stableNameByOriginal.get(name)
        : name;
    }

    function isSmartRenamedType(group) {
      if (!group || typeof group.type !== "string") return false;
      const type = group.type.toLowerCase();
      return type === "url-test" || type === "load-balance";
    }
    ...
    for (const name of required) {
      ...
      let engineName = base;
      let index = 2;
      while (reservedNames.has(engineName)) engineName = `${base} ${index++}`;
      engineNameByOriginal.set(name, engineName);
    ...
    for (const original of ORIGINAL_PROXY_GROUPS) {
      ...
      const stable = stableName(original.name);

      if (Array.isArray(cloned.proxies)) {
        cloned.proxies = cloned.proxies.map(stableName);
      }
      if (typeof cloned["dialer-proxy"] === "string") {
        cloned["dialer-proxy"] = stableName(cloned["dialer-proxy"]);
      }

      if (engineNameByOriginal.has(original.name)) {
        const engineName = engineNameByOriginal.get(original.name);
        cloned.name = engineName;

        // Stable public/dependency name. Smart Override will rename only the inner
        // url-test/load-balance group and will rewrite this proxies reference.
        groups.push({
          name: stable,
          type: "select",
          proxies: [engineName],
